...
{
    int left, right, balance_pwm, velocity_pwm, turn_pwm;
    if (GPIO_Pin != MPU6050_Int_Pin) return;
    Get_Angle(GET_Angle_Way);
    left = Read_Encoder(MOTOR_ID_ML);
    right = -Read_Encoder(MOTOR_ID_MR);
    Get_Velocity_Form_Encoder(left, right);
    balance_pwm = Balance_PD(Angle_Balance, Gyro_Balance);
    velocity_pwm = Velocity_PI(left, right);
    turn_pwm = Turn_PD(Gyro_Turn);
    Motor_Left = PWM_Limit(PWM_Ignore(balance_pwm + velocity_pwm + turn_pwm), 2600, -2600);
    Motor_Right = PWM_Limit(PWM_Ignore(balance_pwm + velocity_pwm - turn_pwm), 2600, -2600);
    if (!Turn_Off(Angle_Balance, battery)) Set_Pwm(Motor_Left, Motor_Right);
}
